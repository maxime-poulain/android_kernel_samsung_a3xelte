...
bool fimc_is_ois_check_fw_impl(struct fimc_is_core *core)
{
	...
	info("[DEBUG] OIS Cal Data %d bite\n", FIMC_IS_OIS_CAL_DATA_SIZE);
	for (i = 0; i < FIMC_IS_OIS_CAL_DATA_SIZE; i++) {
		printk("0x%02X ", buf[EEP_HEADER_OIS_CAL_DATA_ADDR + i]);
		if (((i + 1) % 10) == 0)
			printk("\n");
	}
	info("[DEBUG] OIS Shift Data %d bite\n", FIMC_IS_OIS_SHIFT_DATA_SIZE);
	for (i = 0; i < FIMC_IS_OIS_SHIFT_DATA_SIZE; i++) {
		printk("0x%02X ", buf[EEP_HEADER_OIS_SHIFT_DATA_ADDR + i]);
		if (((i + 1) % 10) == 0)
			printk("\n");
	}
	info("-- OIS DEBUG -- \n");
...
void fimc_is_ois_fw_update_impl(struct fimc_is_core *core)
{
...
#if defined(OIS_TEST_WITHOUT_ISP_CONTROL)
	...
	/* OIS Mode */
	ret = fimc_is_ois_i2c_write(core->client1, 0x6021, 0x7B);
	ret |= fimc_is_ois_read_status(core);

	/* Compensation angle */
	ret |= fimc_is_ois_i2c_write(core->client1, 0x6025, 0x40);
	ret |= fimc_is_ois_read_status(core);
