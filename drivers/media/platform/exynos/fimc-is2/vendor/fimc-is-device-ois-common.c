...
#include "fimc-is-device-ois-bu24219.h"
#endif
...
int fimc_is_ois_reset(struct i2c_client *client)
{
	...
	if(gpio->use_i2c_pinctrl) {
		pin_config_set(gpio->pinname, gpio->reset, PINCFG_PACK(PINCFG_TYPE_FUNC, FUNC_OUTPUT));
		pin_config_set(gpio->pinname, gpio->reset, PINCFG_PACK(PINCFG_TYPE_FUNC, FUNC_OUTPUT));

		pin_config_set(gpio->pinname, gpio->reset, PINCFG_PACK(PINCFG_TYPE_DAT, 0));
		msleep(2);
		pin_config_set(gpio->pinname, gpio->reset, PINCFG_PACK(PINCFG_TYPE_DAT, 1));
		msleep(2);
	}
...
int fimc_is_ois_parse_dt(struct i2c_client *client)
{
	...
	if(gpio->use_i2c_pinctrl) {
		...
		ret = of_property_read_string(np, "fimc_is_ois_reset", (const char **)&gpio->reset);
		if (ret) {
			err("ois gpio: fail to read, ois_parse_dt\n");
			ret = -ENODEV;
			goto p_err;
		...
		info("[OIS] sda = %s, scl = %s, pinname = %s, pinfunc_on = %d, pinfunc_off = %d, reset = %s\n",
				gpio->sda, gpio->scl, gpio->pinname, gpio->pinfunc_on, gpio->pinfunc_off, gpio->reset);
	}
