...
#endif
		.bitwidth = DMA_INPUT_BIT_WIDTH_8BIT,
		.order = DMA_INPUT_ORDER_CrCb,
		.plane = DMA_INPUT_PLANE_2,
		.width = DEFAULT_PREVIEW_STILL_WIDTH,
		.format = DMA_INPUT_FORMAT_YUV420,
		.height = DEFAULT_PREVIEW_STILL_HEIGHT,
...
static int fimc_is_itf_init_process_start(struct fimc_is_device_ischain *device)
{
	...
	if ((path->group[GROUP_SLOT_FD] != GROUP_ID_INVALID))
		group |= (GROUP_ID(path->group[GROUP_SLOT_FD]) & GROUP_ID_PARM_MASK);
...
#ifdef ENABLE_FD_DMA_INPUT
	dma_input = fimc_is_itf_g_param(device, NULL, PARAM_FD_DMA_INPUT);
	dma_input->width = scp_param->dma_output.width;
	dma_input->height = scp_param->dma_output.height;
...
#ifdef ENABLE_FD_SW
	fd_config = fimc_is_itf_g_param(device, NULL, PARAM_FD_CONFIG);
	fimc_is_lib_fd_size_check(device->fd_lib, fd_config, &device->scp.output,
					scp_param->dma_output.width, scp_param->dma_output.height);
...
#ifdef ENABLE_FD_DMA_INPUT
	device->fd_shot = kzalloc(sizeof(struct fd_shot), GFP_KERNEL);
	if (!device->fd_shot) {
...
static int fimc_is_ischain_open(struct fimc_is_device_ischain *device)
{
	...
	imemory->kvaddr_fdshot	= minfo->kvaddr_fdshot;
#endif
	...
	struct fimc_is_video_ctx *vctx)
{
	...
	for (i = 0; i < MAX_FD_BUF; i++) {
		...
		temp->uctl.scalerUd.sourceAddress[2] = 0;
		temp->uctl.scalerUd.sourceAddress[3] = 0;
	}

	ret = fimc_is_itf_unmap(device, GROUP_ID(GROUP_ID_FD));
	if (ret)
	...
	u32 *indexes)
{
	int ret = 0;
	struct param_dma_output *dma_output;
	u32 request_format, hw_format, hw_bitwidth, hw_order;
	...
	if ((request_format == V4L2_PIX_FMT_SBGGR10) || (request_format == V4L2_PIX_FMT_SBGGR12)) {
		...
		hw_order = DMA_OUTPUT_ORDER_GB_BG;
		hw_bitwidth = DMA_OUTPUT_BIT_WIDTH_12BIT;
	} else if (request_format == V4L2_PIX_FMT_YUYV) {
		hw_format = DMA_OUTPUT_FORMAT_YUV422;
		hw_order = DMA_OUTPUT_ORDER_YYCbCr;
	...
	dma_output = fimc_is_itf_g_param(device, frame, PARAM_3AC_VDMA_OUTPUT);
	dma_output->cmd = DMA_OUTPUT_COMMAND_ENABLE;
	dma_output->format = hw_format;
	...
	*lindex |= LOWBIT_OF(PARAM_3AC_VDMA_OUTPUT);
	*hindex |= HIGHBIT_OF(PARAM_3AC_VDMA_OUTPUT);
	(*indexes)++;
	...
	u32 *indexes)
{
	...
	taa_vdma4_output = fimc_is_itf_g_param(device, frame, PARAM_3AC_VDMA_OUTPUT);
...
#ifdef ENABLE_FD_DMA_INPUT
		device->fd_shot->fd_cfg[ldr_frame->fcount%MAX_FD_BUF].otwidth = otparm.w;
		device->fd_shot->fd_cfg[ldr_frame->fcount%MAX_FD_BUF].otheight = otparm.h;
#endif
		if (IS_NULL_CROP(incrop))
			*incrop = inparm;
...
static int fimc_is_ischain_scp_cfg(struct fimc_is_device_ischain *device,
	struct fimc_is_frame *frame,
	u32 width,
	u32 height,
	u32 *lindex,
...
#ifdef ENABLE_FD_SW
static int fimc_is_ischain_vra_tag(struct fimc_is_device_ischain *device,
		struct fimc_is_subdev *subdev,
		struct fimc_is_frame *ldr_frame)
{
	int ret = 0;
	...
	uctl = &ldr_frame->shot->uctl;

	if (!test_bit(FIMC_IS_SUBDEV_RUN, &device->vra.state))
		goto vra_exit;

#ifdef DBG_STREAMING
	mdbgd_ischain("FD TAG(request %d)\n", device, node->request);
#endif
	...
	u32 *indexes)
{
	...
	*hindex |= HIGHBIT_OF(PARAM_DRC_OTF_OUTPUT);
	...
	if (group->subdev[ENTRY_SCP])
		fimc_is_ischain_scp_cfg(device,frame, width, height, lindex, hindex, indexes);
#endif
...
#ifdef ENABLE_FD_DMA_INPUT
			device->fd_shot->fd_cfg[frame->fcount%MAX_FD_BUF].fd_bypass = true;
#endif
		} else {
			if (!test_bit(FIMC_IS_SUBDEV_RUN, &vra->state)) {
...
#ifdef ENABLE_FD_DMA_INPUT
			device->fd_shot->fd_cfg[frame->fcount%MAX_FD_BUF].fd_bypass = false;
#endif
		}
	}
#endif
...
#ifdef ENABLE_FD_SW
	if (test_bit(FIMC_IS_SUBDEV_START, &device->vra.state)) {
		ret = fimc_is_ischain_vra_tag(device, &device->vra, frame);
		if (ret) {
			merr("fimc_is_ischain_vrfa_tag fail(%d)", device, ret);
			goto p_err;
		}
	}
#endif
	...
	struct fimc_is_frame *check_frame)
{
...
#ifdef ENABLE_FD_DMA_INPUT
		if(device->fd_shot->status == false){
			for(i=0;i<MAX_FD_BUF;i++){
				fimc_is_itf_map(device, GROUP_ID(GROUP_ID_FD), device->imemory.dvaddr_fdshot[i], sizeof(struct camera2_shot));
				device->fd_shot->status = true;
			}
		}
#endif
	...
	if(test_bit(FIMC_IS_SUBDEV_RUN, &device->vra.state)) {
		...
		memset(&frame->shot->uctl.fdUd, 0, sizeof(struct camera2_fd_uctl));
	}
