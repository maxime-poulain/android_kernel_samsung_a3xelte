	...
	struct fimc_is_device_ischain *device)
{
	...
	path->group[GROUP_SLOT_FD] = GROUP_ID_FD;
#endif

p_err:
	minfo(" =STM CFG===============\n", device);
	minfo(" %s", device, fimc_is_dmsg_print());
	minfo(" DEVICE GRAPH : %X %X %X %X\n", device, path->group[0], path->group[1], path->group[2], path->group[3]);
	minfo(" =======================\n", device);
	return ret;
}
	...
	struct fimc_is_frame *frame)
{
	...
	if(group->id == GROUP_ID_3AA0){
		int fd_index = (frame->fcount) % MAX_FD_BUF;

		temp = (struct camera2_shot *)device->imemory.kvaddr_fdshot[fd_index];
		memcpy(temp, frame->shot, sizeof(struct camera2_shot));
		temp->uctl.scalerUd.sourceAddress[0] = frame->shot->uctl.scalerUd.scpTargetAddress[0];
		temp->uctl.scalerUd.sourceAddress[1] = frame->shot->uctl.scalerUd.scpTargetAddress[1];
		temp->uctl.scalerUd.sourceAddress[2] = frame->shot->uctl.scalerUd.scpTargetAddress[2];
		temp->uctl.scalerUd.sourceAddress[3] = frame->shot->uctl.scalerUd.scpTargetAddress[3];

		/* Cache Flush */
		vb2_ion_sync_for_device(device->imemory.fw_cookie,
			device->imemory.dvaddr_fdshot[fd_index] - device->imemory.dvaddr,
...
#ifdef ENABLE_FD_SW
	/* It can be changed according to fimc-is data path. */
	vra = group->subdev[ENTRY_VRA];
	if (!vra)
		goto vra_pass;

	if (!group->device->fd_lib) {
		merr("fimc_is_fd_lib is NULL", group);
		goto vra_pass;
	}

	lib_data = group->device->fd_lib->lib_data;
	if (test_bit(FD_LIB_CONFIG, &group->device->fd_lib->state)) {
		fdstats = &frame->shot->dm.stats;
		fdstats->faceDetectMode = FACEDETECT_MODE_FULL;

		if(!test_bit(FD_LIB_RUN, &group->device->fd_lib->state)) {
#ifndef ENABLE_FD_DMA_INPUT
			fimc_is_lib_fd_trigger(group->device->fd_lib);
#endif
			if (&frame->shot->udm.fd != NULL) {
				fd_dm = &frame->shot->udm.fd;
				lib_data->data_fd_lib->sat = fd_dm->vendorSpecific[0];
			}

			lib_data->fd_uctl.faceDetectMode = FACEDETECT_MODE_FULL;
