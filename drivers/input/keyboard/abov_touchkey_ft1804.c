...
static void touchkey_sar_sensing(struct abov_tk_info *info, int on)
{
	...
	if(on==0)	cmd = CMD_ON;
	else	cmd = CMD_OFF;		//EarJack inserted

	tk_debug_info(true, &client->dev, "[TK] %s : %d\n", __func__, !on);
...
#if defined(CONFIG_PM) && !defined(FEATURE_GRIP_FOR_SAR)
...
#if defined(CONFIG_BATTERY_SAMSUNG) && !defined(FEATURE_GRIP_FOR_SAR)
	if (lpcharge == 1) {
