...
static struct {
	 ...
	 * PLL M, P, S
	 */
	APLL_FREQ(1800000, 0, 0, 7, 7, 2, 7, 3, 7, 7, 276, 4, 0),
	APLL_FREQ(1700000, 0, 0, 7, 7, 2, 7, 3, 7, 7, 260, 4, 0),
		...
		case CPU_ONLINE:
			...
			if (cluster == CL_ONE) {
				...
				if (cpumask_weight(&mask) == 1)
					pm_qos_update_request(&cluster_qos_max[CL_ONE], apll_freq[1].freq / 1000);
			}
			break;
		}
	}
...
static int exynos_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	if (samsung_rev() == EXYNOS7580_REV_0) {
		...
		policy->cpuinfo.max_freq = EXYNOS7580_MAX_FREQ;
