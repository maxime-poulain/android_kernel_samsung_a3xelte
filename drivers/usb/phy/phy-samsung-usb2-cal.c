...
void samsung_exynos_cal_dwc_usb2phy_enable(void __iomem *regs_base, u32 refclkfreq)
{
	u32 phyutmi;
	u32 phyclkrst;
	u32 hostplltune;
...
#ifdef CONFIG_K5_REV01_USB_WA
	writel(0x600, regs_base + SAMSUNG_PHYUTMI);
#endif
...
void samsung_exynos_cal_dwc_usb2phy_tune(int cpu_type, void __iomem *regs_base,
				enum samsung_usb_phy_type phy_type)
{
	if (cpu_type == TYPE_EXYNOS7580) {
		u32 phyparam0;

		phyparam0 = readl(regs_base + SAMSUNG_PHYPARAM0);
...
#if defined(CONFIG_USB_PHY_S5NEO)
		...
		case USB_PHY_TYPE_HOST:
			...
			writel(phyparam0, regs_base + SAMSUNG_PHYPARAM0);
			printk(KERN_DEBUG "usb: set PHYPARAM0 for host mode =0x%x\n", phyparam0);
			break;
		}
	} else {
		pr_err("There are no matching cpu type\n");
	}
	return;
}
