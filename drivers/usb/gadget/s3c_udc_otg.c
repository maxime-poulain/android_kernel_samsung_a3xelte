...
static void udc_disable(struct s3c_udc *dev)
{
	...
	if (dev->phy)
		...
		pdata->phy_exit(pdev, USB_PHY_TYPE_DEVICE);
...
#if defined(CONFIG_FAKE_BATTERY)
...
static int udc_enable(struct s3c_udc *dev)
{
	...
	if (dev->phy)
		...
		pdata->phy_init(pdev, USB_PHY_TYPE_DEVICE);
	reconfig_usbd();
...
{
	struct s3c_hsotg_plat *pdata = NULL;
